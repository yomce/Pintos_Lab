...
변수(ticks)는 운영체제가 부팅된 이후 지금까지 얼마나 시간이 흘렀는지를 "틱 단위"로 나타내는 값*/
static int64_t ticks;

struct list sleep_list; // 모든 sleep 요청한 스레드 저장 리스트
...
이 코드는 그 타이머를 설정해서, 예를 들어 초당 100번, 1000번 등 원하는 주기만큼 인터럽트를 발생시키게 만듦
동시에 그 인터럽트가 발생했을 때 실행할 함수(보통 timer_interrupt)도 인터럽트 핸들러로 등록함
즉, 운영체제가 시간 개념을 가질 수 있게 해주는 기본 시계 역할을 설정하는 부분*/
void timer_init(void) {
    /* 8254 입력 주파수를 TIMER_FREQ로 나눈 값이며, 가장 가까운 정수로 반올림함 */
    /*8254 타이머 칩은 보통 기본 입력 주파수 1,193,180Hz를 사용
    그런데 핀토스는 이 타이머를 초당 TIMER_FREQ번 인터럽트를 발생시키도록 설정하고 싶어 함.
    (예: #define TIMER_FREQ 100이면 초당 100번 인터럽트)
    divisor(설정할 분주 값) = 1193180 / TIMER_FREQ*/
    uint16_t count = (1193180 + TIMER_FREQ / 2) / TIMER_FREQ;

    outb(0x43, 0x34); /* CW(Control Word): 카운터 0 사용, LSB 먼저 그다음 MSB, 모드 2, 이진 모드 */
...
CPU가 얼마나 빠른지 체크해서, 잠깐 멈추고 싶을 때 몇 번 루프 돌면 될지 계산하는 함수*/
void timer_calibrate(void) {
    ...
    /* loops_per_tick 값을 타이머 틱 하나보다 작은 값 중 가장 큰 2의 거듭제곱으로 근사함 */
