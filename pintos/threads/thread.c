...
return ta->priority > tb->priority; // 높은 우선순위가 먼저
}
...
void thread_sleep(int64_t ticks) {
  ...
  old_level = intr_disable(); //인터럽트 차단
...
/* 블록된 스레드 T를 실행 준비 상태(ready-to-run)로 전환함.
   T가 블록된 상태가 아니라면 오류임.
   (현재 실행 중인 스레드를 준비 상태로 만들고 싶다면 thread_yield()를 사용할
   것)

   이 함수는 현재 실행 중인 스레드를 선점하지 않음.
   이는 중요한데, 호출자가 인터럽트를 비활성화한 상태라면
   스레드를 원자적으로 unblock하고 다른 데이터를 수정할 수 있다고 기대할 수 있기
   때문임. */
void thread_unblock(struct thread *t) {
  ...
  list_insert_ordered(&ready_list, &t->ready_elem, thread_compare_priority, NULL);
...
static void kernel_thread(thread_func *function, void *aux) {
  ...
  intr_enable(); /* 스케줄러는 인터럽트가 꺼진 상태에서 실행되므로, 여기서
                    인터럽트를 활성화함 */
...
static void schedule(void) {
  ...
  if (curr != next) {
    /* 전환 이전에 실행되던 스레드가 종료 상태(THREAD_DYING)라면,
            ...
            실제로 파괴(해제)하는 로직은 다음 스케줄의 시작 시점(schedule()의
       시작)에서 실행됨. */
